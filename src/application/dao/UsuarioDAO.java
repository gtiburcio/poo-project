...
public class UsuarioDAO implements IDAO {
    ...
    public List<IModel> findAll() throws SQLException {
        ...
        while (rs.next()) {
            Usuario usuario = Usuario.builder().id(rs.getLong("id_usuario")).nome(rs.getString("nome"))
                    .login(rs.getString("login")).senha(rs.getString("senha"))
                    .perfil(Perfil.builder().id(rs.getLong("id_perfil")).build()).build();
    ...
    public IModel findById(long id) throws Exception {
        ...
        ResultSet rs = pstm.executeQuery();
        if (rs.next()) {
            return Usuario.builder().id(rs.getLong("id_usuario")).nome(rs.getString("nome"))
                    .login(rs.getString("login")).senha(rs.getString("senha"))
                    .perfil(Perfil.builder().id(rs.getLong("id_perfil")).build()).build();
        }
        return null;
    ...
    public Usuario findByLogin(String login) throws Exception {
        ...
        ResultSet rs = pstm.executeQuery();
        if (rs.next()) {
            return new Usuario(rs.getLong("id_usuario"), rs.getString("nome"), rs.getString("login"),
                    rs.getString("senha"), Perfil.builder().id(rs.getLong("id_perfil")).build());
        }
        return null;
    }
}
